...
{if ($byjuno_tmx == true)}
    <script>
        window.addEventListener('load', function () {
            window.setTimeout(function() {
                ...
                s.async = true;
                s.onload = function() {
                    threatmetrix.profile("prof4rs.cembrapay.ch", "{$byjuno_tmx_org_id}", "{$byjuno_tmx_session_id}");
                };
    ...
    <noscript>
        <iframe style="width: 100px; height: 100px; border: 0; position: absolute; top: -5000px;" src="https://prof4rs.cembrapay.ch/fp/tags?org_id={$byjuno_tmx_org_id}&session_id={$byjuno_tmx_session_id}"></iframe>
    </noscript>
{/if}
...
{if $is_api}
    ...
    <div id="panel-form">
        <div class="row ">
                        ...
                        <div class="col col-12">
                            {foreach from=$selected_payments_installment item=s_payment}
                                <input type="radio" name="byjuno_payment"
                                       value="{$s_payment.id}" {if $s_payment.id == $selected_payment_installment} checked="checked"{/if}>
                                &nbsp;{$s_payment.name}
                                <br/>
                            {/foreach}
                        </div>
            ...
            {/if}
            {if (count($selected_payments_installment) == 1)}
                <input type="hidden" name="byjuno_payment" value="{$selected_payments_installment[0].id}">
            {/if}
...
{else}
    <input type="hidden" name="byjuno_form" value="0">
    <div id="panel-form">
        <div class="row">
            <div class="col col-12">
                <div class="h2">
